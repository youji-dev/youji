<template>
    <el-dialog v-model="isVisable" width="500" :before-close="beforeClose" :title="i18n.t('deleteTicketTitle')"
        v-on:closed="beforeClose">
        <p style="font-weight: bold;">{{ ticket?.title }}</p>
        <span>{{ $t("deleteTicketDescription") }}</span>
        <template #footer>
            <div class="dialog-footer">
                <el-button @click="isVisable = false" type="default">{{ $t("close") }}</el-button>
...
const isVisable = defineModel<boolean>("visable", { required: true });
