<template>
  <div class="w-full h-full py-6 lg:py-0 px-6 mt-6 lg:mt-0" style="height: calc(100vh - 72px)" :style="{ width: width }"
    ...
    <div class="flex flex-row items-center justify-end w-full py-2">
      <div class="w-1/2 flex justify-center items-center md:w-1/3 lg:w-1/5">
        <el-input v-model="search" class="w-full mr-1" :placeholder="$t('searchVerb')" :prefix-icon="Search"
          @change="fetchTicketsFromStart(true)" />
        <el-button class="ml-1" type="primary" :icon="ElIconSearch" @click="fetchTicketsFromStart(true)"
          :loading="searchLoading" round></el-button>
      </div>
    </div>
    <div
      ...
      <el-table v-if="!loading && !pageLoading && tickets.length > 0" :data="parsedTickets"
        ...
        <el-table-column class="hidden lg:block" prop="author" filter-class-name="Author" :label="$t('username')"
          width="150" show-overflow-tooltip sortable />
        <el-table-column prop="title" filter-class-name="Title" :label="$t('title')" min-width="250"
          show-overflow-tooltip sortable />
        <el-table-column prop="state.name" filter-class-name="State" :label="$t('status')"
          :filters="statusOptions.map((opt: state) => { return { text: opt.name, value: opt.id } })"
          :filter-method="filterTag" filter-placement="bottom-end" width="200" sortable>
          <template #default="scope">
            <div class="flex justify-start items-center">
              <ColoredSelect :change-callback="updateTicketStatus" :change-callback-params="[scope.row.id]"
                :add-current-value-to-callback="true" :current="new ColoredSelectOption(
                  scope.row.state,
                  ...
                  " :options="statusOptions.map((opt) => {
                    ...
                    " :keyText="'id'" :labelText="'name'" :id="scope.row.id"></ColoredSelect>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="building.name" filter-class-name="Building" class="hidden lg:block"
          :label="$t('building')" width="150" show-overflow-tooltip sortable />
        <el-table-column prop="room" filter-class-name="Room" class="hidden lg:block" :label="$t('room')" width="100"
          show-overflow-tooltip sortable />
        <el-table-column prop="priority.name" filter-class-name="Priority" :label="$t('priority')"
          :filters="priorityOptions.map((opt: priority) => { return { text: opt.name, value: opt.id } })"
          :filter-method="filterTag" filter-placement="bottom-end" width="200" sortable>
          <template #default="scope">
            <div class="flex justify-start items-center">
              <ColoredSelect :change-callback="updateTicketPriority" :change-callback-params="[scope.row.id]"
                :add-current-value-to-callback="true" :current="new ColoredSelectOption(
                  scope.row.priority,
      ...
      <el-pagination class="mr-auto" v-if="!loading && tickets.length > 0" layout="prev, pager, next"
